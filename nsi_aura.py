...
global_links = [
    NetworkLink(id=1, name='MOXY EXA Atlantic North 100G', linkid=31, svlanid=190, evlanid=190, domain=DEFAULT_LINK_DOMAIN),
    NetworkLink(id=2, name='Tata TGN-Atlantic South 100G', linkid=32, svlanid=190, evlanid=190, domain=DEFAULT_LINK_DOMAIN),
    NetworkLink(id=3, name='AquaComms AEC-1 South 100G', linkid=33, svlanid=190, evlanid=190, domain=DEFAULT_LINK_DOMAIN),
...
global_reservations = [
    Reservation(id=1, connectionId=DUMMY_CONNECTION_ID_STR,description="Dummy reservation",startTime="2024-11-07T15:53:32+00:00",endTime="2024-11-07T15:53:36+00:00",sourceSTP="asd001b-jnx-06_eth0",destSTP="mon001a-nok-01_eth1",requesterNSA="urn:ogf:network:anaeng.global:2024:nsa:nsi-aura",reservationState="ReserveHeld",lifecycleState="Created",dataPlaneStatus="true" ),
]
...
def nsi_load_parsed_soap_into_reservations_model(resdictlist):
    ...
    for connid in resdictlist.keys():
        ...
        if reserve_dict[S_RESERVATION_STATE_TAG] == NSI_RESERVATION_FAILED_STATE or reserve_dict[S_RESERVATION_STATE_TAG] == NSI_RESERVATION_TIMEOUT_STATE:
            ...
            reservation = Reservation(id=reserve_dict[FASTUID_ID_KEY],connectionId=connid,description=reserve_dict[S_DESCRIPTION_TAG],startTime=reserve_dict[S_STARTTIME_TAG],endTime=reserve_dict[S_ENDTIME_TAG],sourceSTP=reserve_dict[S_SOURCE_STP_TAG],destSTP=reserve_dict[S_DEST_STP_TAG],requesterNSA=reserve_dict[S_REQUESTER_NSA_TAG],reservationState=reserve_dict[S_RESERVATION_STATE_TAG],lifecycleState=reserve_dict[S_LIFECYCLE_STATE_TAG],dataPlaneStatus=reserve_dict[S_DATAPLANE_STATUS_TAG])
        else:
            reservation = Reservation(id=reserve_dict[FASTUID_ID_KEY],connectionId=connid,description=reserve_dict[S_DESCRIPTION_TAG],startTime=reserve_dict[S_STARTTIME_TAG],endTime=reserve_dict[S_ENDTIME_TAG],sourceSTP=reserve_dict[S_SOURCE_STP_TAG],destSTP=reserve_dict[S_DEST_STP_TAG],requesterNSA=reserve_dict[S_REQUESTER_NSA_TAG],reservationState=reserve_dict[S_RESERVATION_STATE_TAG],lifecycleState=reserve_dict[S_LIFECYCLE_STATE_TAG],dataPlaneStatus=reserve_dict[S_DATAPLANE_STATUS_TAG])
...
def add_links_table(heading,clickurl,local_links) -> list[AnyComponent]:
    ...
    return [
            ...
            c.Paragraph(text="NSI supports Explicit Route Objects (ERO), allowing link selection",class_name="+ text-success"),
            c.Table(
                data=local_links,
                # define two columns for the table
                columns=[
    ...
    return [
        c.Page(  # Page provides a basic container for components
            components=[
                ...
                c.Table(data_model=Reservation,
...
def fastapi_landing_page(request: Request) -> list[AnyComponent]:
    ...
    if auth_bool:
        ...
        auth_str = "You are accessing a system illegally according to the Dutch Wet Computer Criminaliteit. You MUST go away!"
    ...
    return [
        c.Page(  # Page provides a basic container for components
            components=[
                ...
                c.Link(components=[c.Paragraph(text='1. Mail NOC of domain A and Z to determine Customer VLAN IDs and add matching endpoints to NSI domain topologies')], on_click=GoToEvent(url=mailto_noc_url)),
                c.Link(components=[c.Paragraph(text='2. Reload topologies of domains from NSI-DDS')], on_click=GoToEvent(url=reload_topos_url)),
                c.Link(components=[c.Paragraph(text='3. Select endpoints and link')], on_click=GoToEvent(url=selecta_url)),
                c.Heading(text="Overview", level=3),
                c.Link(components=[c.Paragraph(text='Query existing connections from NSI Aggregator')], on_click=GoToEvent(url=query_url)),
                create_footer(),
            ]
        ),
    ]
...
async def orchestrator_general_callback(request: Request):
    ...
    if content_type == 'application/xml' or content_type == 'text/xml' or content_type == 'text/xml;charset=utf-8' or content_type == 'text/xml; charset=UTF-8' or content_type.startswith('text/xml'):
        try:
...
def fastapi_nsi_reserve(epida: int,epidz: int,linkid:int) -> list[AnyComponent]:
    ...
    try:
        ...
        if ONLINE:
            ...
            reserve_reply_dict = nsi_reserve(reserve_templstr,global_soap_provider_url,correlation_uuid_py,orch_reply_to_url,global_provider_nsa_id,endpointa.name,endpointa.svlanid,endpointz.name,endpointz.evlanid,link.name,link.id,duration_td)
...
def fastapi_nsi_reserve_commit(connid: str) -> list[AnyComponent]:
    ...
    try:
        ...
        if ONLINE:
            ...
            reserve_commit_reply_dict = nsi_reserve_commit(reserve_commit_templstr,global_soap_provider_url,SERVER_URL_PREFIX,global_provider_nsa_id,clean_connection_id_str)

        # RESTFUL: Do Not Store (TODO or for security)
        # TODO NEWCALLBACK
        orch_reply_to_url = SERVER_URL_PREFIX+"/reserve-commit-callback/?corruuid="+reserve_commit_reply_dict["correlationId"]+"&globresuuid="+reserve_commit_reply_dict["globalReservationId"]+"&connid="+clean_connection_id_str
    ...
    return [
        c.Page(
            components=[
                ...
                c.Paragraph(text=faultstring, class_name=cssclassname),
                c.Link(components=[c.Paragraph(text='Simulate async Reserve Commit Callback')], on_click=GoToEvent(url=orch_reply_to_url)),
                c.Link(components=[c.Paragraph(text='Back to landing page')], on_click=GoToEvent(url=root_url)),
            ]
        ),
    ]
...
def fastapi_nsi_reserve_commit_callback(corruuid: str,globresuuid: str, connid: str) -> list[AnyComponent]:
    ...
    try:
        ...
        if ONLINE:
            ...
            provision_reply_dict = nsi_provision(provision_templstr,global_soap_provider_url,SERVER_URL_PREFIX,global_provider_nsa_id,expect_global_reservation_uuid_py,clean_connection_id_str)

        # RESTFUL: Do Not Store (TODO or for security)
        orch_reply_to_url = SERVER_URL_PREFIX+"/provision-callback/?corruuid="+provision_reply_dict["correlationId"]+"&globresuuid="+provision_reply_dict["globalReservationId"]+"&connid="+clean_connection_id_str
    ...
    return [
        c.Page(
            components=[
                ...
                c.Heading(text="Got reply for connection "+clean_connection_id_str, level=4),  # NOT FROM URL QUERY
                c.Paragraph(text=faultstring, class_name=cssclassname),
                c.Link(components=[c.Paragraph(text='Simulate async Provision Callback')], on_click=GoToEvent(url=orch_reply_to_url)),
                c.Link(components=[c.Paragraph(text='Back to landing page')], on_click=GoToEvent(url=root_url)),
            ]
        ),
    ]
    ...
    return [
        c.Page(
            components=[
                ...
                c.Link(components=[c.Paragraph(text='Query existing connections from NSI Orchestrator')], on_click=GoToEvent(url=query_url)),
                c.Link(components=[c.Paragraph(text='Back to Landing Page')], on_click=GoToEvent(url=root_url)),
            ]
        ),
    ]
...
def fastapi_nsi_connections_query() -> list[AnyComponent]:
    ...
    try:
        ...
        if ONLINE:
            ...
            resdictlist = nsi_connections_query(query_summary_sync_templstr,global_soap_provider_url,SERVER_URL_PREFIX,global_provider_nsa_id)
    ...
    return [
        c.Page(
            components=[
                ...
                c.Heading(text="Overview", level=3),
                c.Link(components=[c.Text(text='Redo query existing connections from NSI Aggregator')], on_click=GoToEvent(url=query_url)),
...
def fastapi_nsi_terminate(connid: str) -> list[AnyComponent]:
    ...
    try:
        ...
        if ONLINE:
            ...
            terminate_reply_dict = nsi_terminate(terminate_templstr,global_soap_provider_url,SERVER_URL_PREFIX,global_provider_nsa_id,clean_connection_id_str)
        ...
        # RESTFUL: Do Not Store (TODO or for security)
        orch_reply_to_url = SERVER_URL_PREFIX+"/terminate-callback/?corruuid="+terminate_reply_dict["correlationId"]+"&connid="+clean_connection_id_str
    ...
    return [
        c.Page(
            components=[
                ...
                c.Heading(text="Got reply for connection "+clean_connection_id_str, level=4),  # NOT FROM URL QUERY
                c.Paragraph(text=faultstring, class_name=cssclassname),
                c.Link(components=[c.Paragraph(text='Simulate async Terminate Callback')], on_click=GoToEvent(url=orch_reply_to_url)),
                c.Link(components=[c.Paragraph(text='Back')], on_click=BackEvent()),
            ]
        ),
    ]
...
def fastapi_nsi_release(connid: str) -> list[AnyComponent]:
    ...
    try:
        ...
        if ONLINE:
            ...
            release_reply_dict = nsi_release(release_templstr,global_soap_provider_url,SERVER_URL_PREFIX,global_provider_nsa_id,clean_connection_id_str)
        ...
        # RESTFUL: Do Not Store (TODO or for security)
        orch_reply_to_url = SERVER_URL_PREFIX+"/release-callback/?corruuid="+release_reply_dict["correlationId"]+"&connid="+clean_connection_id_str
    ...
    return [
        c.Page(
            components=[
                ...
                c.Heading(text="Got reply for connection "+clean_connection_id_str, level=4),  # NOT FROM URL QUERY
                c.Paragraph(text=faultstring, class_name=cssclassname),
                c.Link(components=[c.Paragraph(text='Simulate async release Callback')], on_click=GoToEvent(url=orch_reply_to_url)),
                c.Link(components=[c.Paragraph(text='Back')], on_click=BackEvent()),
            ]
        ),
    ]
...
def fastapi_nsi_reserve_timeout_ack(connid: str) -> list[AnyComponent]:
    ...
    try:
        ...
        if ONLINE:
            ...
            reserve_timeout_ack_reply_dict = nsi_reserve_timeout_ack(reserve_timeout_ack_templstr,global_soap_provider_url,SERVER_URL_PREFIX,global_provider_nsa_id,clean_connection_id_str)
        ...
        # RESTFUL: Do Not Store (TODO or for security)
        orch_reply_to_url = SERVER_URL_PREFIX+"/reserve_timeout_ack-callback/?corruuid="+reserve_timeout_ack_reply_dict["correlationId"]+"&connid="+clean_connection_id_str
    ...
    return [
        c.Page(
            components=[
                ...
                c.Heading(text="Got reply for connection "+clean_connection_id_str, level=4),  # NOT FROM URL QUERY
                c.Paragraph(text=faultstring, class_name=cssclassname),
                c.Link(components=[c.Paragraph(text='Simulate async reserve_timeout_ack Callback')], on_click=GoToEvent(url=orch_reply_to_url)),
                c.Link(components=[c.Paragraph(text='Back')], on_click=BackEvent()),
            ]
        ),
    ]
...
def fastapi_nsi_query_recursive(connid: str) -> list[AnyComponent]:
    ...
    try:
        ...
        if ONLINE:
            ...
            query_recursive_reply_dict = nsi_query_recursive(query_recursive_templstr,global_soap_provider_url,orch_reply_to_url,global_provider_nsa_id,clean_connection_id_str)
        ...
        #poll_url = '/poll/reserve/'
        #poll_url = "/poll/reserve/?corruuid="+corruuid+"&connid="+connid
        poll_url = generate_poll_url(FASTAPI_MSGNAME_QUERY_RECURSIVE,clean_correlation_uuid_str,clean_connection_id_str)
    ...
    return [
        c.Page(
            components=[
                ...
                c.Heading(text="Got reply for connection "+clean_connection_id_str, level=4),  # NOT FROM URL QUERY
                c.Paragraph(text=faultstring, class_name=cssclassname),
                c.Link(components=[c.Paragraph(text='Simulate async query-recursive callback by Aggregator')], on_click=GoToEvent(url=orch_reply_to_url)),
    ...
    return [
        c.Page(
            components=[
                ...
                c.Link(components=[c.Paragraph(text='Simulate ReserveTimeoutAck Connection')], on_click=GoToEvent(url=reserve_timeout_ack_url)),
                c.Link(components=[c.Paragraph(text='Query Recursive Connection')], on_click=GoToEvent(url=query_recursive_url)),
            ]
        ),
    ]
